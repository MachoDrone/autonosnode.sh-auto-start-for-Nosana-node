#!/bin/bash
# Automatically start Nosana Node on tty2 for Ubuntu Server and for Ubuntu Desktop where the GUI is likely to already be on tty1.

# Starting Nosana node on tty2 may be convenient for Ubuntu Desktop, but it is highly recommended to run Nosana node
# from a terminal in the GUI for ease of screencaptures/support.

# The objective of this script is to automatically launch Nosana Node on Ubuntu 22.04.
# It will resmble operation of the node as if the user manually logged-in to tty2 and ran the Nosana node start script in the cli.
# This will be mosty familiar to a novice owner/operator who may already be doing the manual tasks with addition to ctrl-alt-F2 to view Nosana node.
# Before running this script, the user will need to type the command: chmod +x autonosnode2.sh
...
draw_box() {
    ...
    echo "┃                                                                          ┃"
    echo "┃                                                                          ┃"
    echo "┃      This script will configure autologin for Nosana node on tty2.       ┃"
    echo "┃                                                                          ┃"
    echo "┃                                                                          ┃"
    ...
    echo "┃          2. Configure autologin for tty2                                 ┃"
...
# Function to configure autologin for tty2
configure_autologin() {
...
uninstall_script() {
    ...
    sudo rm -rf /etc/systemd/system/getty@tty2.service.d/
...
echo "┃  Both simply use the same command:  sudo ./autonosnode2.sh ┃"
echo "┃                                                            ┃"
